...
def idade_e_valida(data_nascimento, data_primeiro_pre_natal): #checa se a idade é valida e se for, retorna a idade
    if(campo_nao_preenchido(data_nascimento) or campo_nao_preenchido(data_primeiro_pre_natal) or data_nao_e_valida(data_nascimento, data_primeiro_pre_natal)):
        return False
...
def primeiro_pre_natal_valido(data_inicio_gestacao, data_primeiro_pre_natal):#checa se é valido e se for retorna qntd de semanas
    if(campo_nao_preenchido(data_inicio_gestacao) or campo_nao_preenchido(data_primeiro_pre_natal) or data_nao_e_valida(data_inicio_gestacao, data_primeiro_pre_natal)):
        return False
